...
-- 6.  Constant product formula must be satisfied (up to fee)
-- 7.  All value being used as input for swapping must come from orderbook V2
-- 8.  Pool datum must remain unchanged
-- 9.  Swapper license token must be present and not be expired
-- 10. Validate that no utxo is spent from scripts other than pool (e.g. batcher)
validateDirectSwap ::
    ...
    let info :: TxInfo
        ...
        licenseDeadline =
          let (Value mp) = txOutValue $ txInInfoResolved licenseInput
              (TokenName license) = case Map.lookup ppSwapperLicenseSymbol mp of
        ...
        checkSwap oldA' oldB' newA' newB' =
          ...
          where
            ...
            -- The uniswap fee is 0.3%; here it is multiplied by 10000, so that the
        ...
        allValueFromV2 :: Bool
        allValueFromV2
          | amountAIn < amountAOut = amountAIn + (amountFromV2 coinA) >= amountAOut
          | otherwise = amountBIn + (amountFromV2 coinB) >= amountBOut
          where
            amountFromV2 :: AssetClass -> Integer
            amountFromV2 coin =
              let v2InAmounts = [assetClassValueOf (txOutValue $ txInInfoResolved i) coin | i <- txInputs, isFromV2 i]
                  isFromV2 i = case toValidatorHash $ txOutAddress $ txInInfoResolved i of
                    Just vh -> vh == v2ValHash
                    _ -> False
               in sum v2InAmounts
     in validPoolValue -- 1.
          ...
          && (after (POSIXTime licenseDeadline) validTimeRange) -- 9.
...
validateApplyPool
  ...
  licenseIndex =
    let info :: TxInfo
        ...
        licenseDeadline =
          let (Value mp) = txOutValue $ txInInfoResolved licenseInput
              (TokenName license) = case Map.lookup ppBatcherLicenseSymbol mp of
                Just i -> case Map.toList i of
                  [(tn, _)] -> tn
                  _ -> error ()
                _ -> error ()
