...
import MuesliSwapPools.Utils.OnChainUtils (assertPoolValue, mustFindScriptDatum')
    ...
    !params =
      FactoryParams
        { fpNftSymbol = mkNFTSymbol,
          ...
          fpOwnerTokenName = mkOwnerTokenName,
          fpBatcherLicenseSymbol = mkBatcherLicenseSymbol,
          fpSwapperLicenseSymbol = mkSwapperLicenseSymbol
        }
...
mkFactoryValidator FactoryParams {..} rawRedeemer rawContext =
  ...
  where
    ...
    hasOwner ownerIndex =
      let ownerInput :: SC.TxInInfo
          ownerInput = txInputs !! ownerIndex
          (Value mp) = SC.txOutValue $ SC.txInInfoResolved ownerInput
          ownerTokenName = case Map.lookup fpBatcherLicenseSymbol mp of
            Just i -> case Map.toList i of
              [(tn, _)] -> tn
              _ -> error ()
            _ -> error ()
       in ownerTokenName == fpOwnerTokenName
